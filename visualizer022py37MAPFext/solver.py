...
import subprocess
import from_asprilo
import Convert_agents_actions_2_asp

VERSION = '0.3.0'  # version 0.3.0 to support MAPF solvers copied from version '0.2.1'
...
class Solver(object):
    def __init__(self):
        ...
        self._parser.add_argument('-m', '--mode',
                            ...
                            type = str, choices=['default', 'incremental', 'interactive', 'online'
                                    , 'astar', 'cbs', 'epea', 'icts', 'macbs', 'cbsh'], default = 'default')
...
class SolverAstar(Solver):
    def __init__(self):
        super(SolverAstar, self).__init__()
    ...
    def on_data(self, data):
        ...
        from_asprilo.from_asprilo_from_viz(viz_instance2solve)
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('solving A* with OD with ID, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/A_Star_WithOD_WithID-MAKESPAN.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('solving A* with OD with ID, Sum of Costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/A_Star_WithOD_WithID.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
        # with open('solved.astar', 'w') as f:
        #     f.write(a_star_output)
        satisfiable = MAPF_output.count('|(')  # part of solved content
        if satisfiable > 0:
            print('solved in {} seconds'.format(MAPF_time))
            asp_plan = Convert_agents_actions_2_asp.convert_actions_from_viz(MAPF_output.split('\n')[:-1])
            # add empty entry to dictionary
            self._to_send[0] = []
            for line in asp_plan:
                self._to_send[0].append(line[:-1])  # removing . at end of rule
            # with open("asprilo_m_outcome.plan") as f:
            #     for line in f:
            #         self._to_send[0].append(line[:-2])
        # send data to the visualizer after solving
            self.send('%$RESET.')
            self.send_step(0)
        else:
...
class SolverCbs(Solver):
    ...
    def on_data(self, data):
        ...
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('solving cbs with bypass with Cardinal Lookahead, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/cbs-withBypass-withCardinalLookahead-MAKESPAN.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('No solver CBS, sum of costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('echo No solver CBS, sum of costs goal'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
...
class SolverEpea(Solver):
    def __init__(self):
        super(SolverEpea, self).__init__()
    ...
    def on_data(self, data):
        ...
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('No solver EPEA* with ID, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run('echo No solver EPEA* with ID, MAKESPAN goal'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('solving EPEA* with ID, sum of costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/EPEAstarWithID.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
...
class SolverICTS(Solver):
    def __init__(self):
        super(SolverICTS, self).__init__()
    ...
    def on_data(self, data):
        ...
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('No solver for ICTS, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run('echo No solver for ICTS, MAKESPAN goal'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('solving ICTS, sum of costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/ICTS_WithID.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
...
class SolverMacbs(Solver):
    def __init__(self):
        super(SolverMacbs, self).__init__()
    ...
    def on_data(self, data):
        ...
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('solving macbs-over-A* with OD-20-withBypass, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run\
                ('dotnet solvers/macbs-over-astarwithod-20-withBypass-withCardinalLookahead-MAKESPAN.dll {} {}'
                 .format('maze.map', 'agents.agents'), stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('No solver for macbs, sum of costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('echo No solver for macbs, sum of costs goal'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
...
class SolverCbsh(Solver):
    def __init__(self):
        super(SolverCbsh, self).__init__()
    ...
    def on_data(self, data):
        ...
        # a_star
        if self._args.encoding == 'MAKESPAN':
            print('No solver for modern cbs, MAKESPAN goal')
            self._solve_start = time.clock()
            content = subprocess.run\
                ('echo No solver for modern cbs, MAKESPAN goal'
                 .format('maze.map', 'agents.agents'), stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        else:
            print('solving modern cbs (cbsh), sum of costs goal')
            self._solve_start = time.clock()
            content = subprocess.run('dotnet solvers/modern-cbs.dll {} {}'.format('maze.map', 'agents.agents')
                                     , stdout=subprocess.PIPE, shell=True)
            MAPF_time = time.clock() - self._solve_start
        MAPF_output = content.stdout.decode('utf-8')
